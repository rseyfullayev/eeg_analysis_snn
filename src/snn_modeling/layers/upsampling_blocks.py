...
import torch
from .residual_blocks import ConvBnSpiking, ConvSpiking


class TimeDistributedUpsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2):
        super(TimeDistributedUpsample, self).__init__()
        self.up = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, bias=False)
    ...
    def forward(self, x):
        if x.dim() == 5:
            ...
            return self.up(x) #self.norm(self.up(x))

class SpikingUpsampleBlock(nn.Module):
    def __init__(self, in_channels, skip_channels, id_channels, out_channels, spike_model=snn.Leaky, **neuron_params):
...
class FinalUpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, spike_model=snn.Leaky, **neuron_params):
        ...
        self.conv1 = ConvSpiking(in_channels, in_channels, kernel_size=3, padding=1, spike_model=spike_model, **neuron_params)
    
        self.up2 = TimeDistributedUpsample(in_channels, out_channels, kernel_size=2, stride=2)
        self.conv2 = ConvSpiking(out_channels, out_channels, kernel_size=3, padding=1, spike_model=spike_model, **neuron_params)
